    ...
    <div class="col-sm-12 col-md-12 col-xl-12">
        ...
        <div class="float-end m-4">
            <div class="border border-secondary p-2 rounded d-inline-block">
                <a href="{{ route('downloadCsv', ['id' => $generation->id]) }}" class="text-decoration-none text-secondary d-flex align-items-center">
        ...
        <form class="card-form form mb-4" action="{{ route('generation-update', ['id' => $generation->id]) }}" method="POST" enctype="multipart/form-data" id="generationForm">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="generation_name"
                    ...
                    value="{{ old('name', $generation->name) }}" placeholder="e.g., Generation 2021" maxlength="100" required>
                @error('name')
                    ...
                    <div class="col-sm-6 col-md-4 col-xl-3 term-card">
                        <div class="card h-100 shadow-sm position-relative">
                            ...
                            <div class="card-body d-flex flex-column align-items-center justify-content-center gap-1">
                                <input type="text" name="term_name[]" value="{{ old('term_name.' . $loop->index, $term->name) }}" class="form-control text-center w-75" placeholder="Term Name" required>
                                <input type="date" name="start_date[]" value="{{ old('start_date.' . $loop->index, $term->start_date) }}" class="form-control w-75" placeholder="Start Date">
                                <input type="date" name="end_date[]" value="{{ old('end_date.' . $loop->index, $term->end_date) }}" class="form-control w-75" placeholder="End Date">
                            </div>
                        </div>
                    </div>
                    @endforeach
            ...
            <div class="d-flex justify-content-between align-items-center mt-4">
                <a href="{{ route('generation') }}" class="btn btn-outline-primary"><i class="bi bi-chevron-left me-1"></i> Back</a>
                <div class="d-flex gap-2">
                    <button type="reset" class="btn btn-outline-secondary"><i class="bi bi-arrow-clockwise me-1"></i> Reset</button>
                    <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i> Update Generation</button>
                </div>
            </div>
        </form>
    </div>
