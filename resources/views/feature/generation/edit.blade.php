        ...
        <div class="col-sm-12 col-md-12 col-xl-12">
            ...
            {{-- <div class="float-end m-4">
                <div class="border border-secondary p-2 rounded d-inline-block">
                    <a href="{{ route('downloadCsv', ['id' => $generation->id]) }}"
                        class="text-decoration-none text-secondary d-flex align-items-center">
                        <i class="bi bi-file-earmark-arrow-down me-2"></i>
                        <span class="browseExport" id="exportCsvTitle">downloadCsv</span>
                    </a>
                </div>
            </div> --}}
            ...
            <form class="card-form form mb-4" action="{{ route('generation-update', ['id' => $generation->id]) }}"
                method="POST" enctype="multipart/form-data" id="generationForm">
                @csrf
                @method('patch')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="generation_name"
                        ...
                        value="{{ old('name', $generation->name) }}" placeholder="e.g., Generation 2021" maxlength="100"
                        required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <div class="d-flex gap-2">
                        <input type="number" name="start_year"
                            ...
                            value="{{ old('start_year', $generation->start_year) }}" required min="2000" max="2100">
                        <span class="d-flex align-items-center">-</span>
                        <input type="number" name="end_year" class="form-control @error('end_year') is-invalid @enderror"
                            placeholder="To (e.g., 2024)" value="{{ old('end_year', $generation->end_year) }}" required
                            min="2000" max="2100">
                    </div>
                    @error('start_year')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-4">
                    ...
                    <small class="form-text text-muted d-block mb-3">
                        Click "Add term" to create terms for this generation. You must add at least one term.
                    </small>
                        ...
                        @foreach ($generation->terms as $term)
                            <div class="col-sm-6 col-md-4 col-xl-3 term-card">
                                <div class="card h-100 shadow-sm position-relative">
                                    ...
                                    {{-- Term Inputs --}}
                                    <div
                                        class="card-body d-flex flex-column align-items-center justify-content-center gap-1">
                                        <input type="text" name="term_name[]"
                                            value="{{ old('term_name.' . $loop->index, $term->name) }}"
                                            class="form-control text-center w-75" placeholder="Term Name" required>
                                        <input type="date" name="start_date[]"
                                            value="{{ old('start_date.' . $loop->index, $term->start_date) }}"
                                            class="form-control w-75" placeholder="Start Date">
                                        <input type="date" name="end_date[]"
                                            value="{{ old('end_date.' . $loop->index, $term->end_date) }}"
                                            class="form-control w-75" placeholder="End Date">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    {{-- Terms Count --}}
                    <div class="mt-4">
                        <small class="text-muted">Terms added: <span
                                id="terms-count">{{ count($generation->terms) }}</span></small>
                    </div>
                </div>

                {{-- Action Buttons --}}
                <div class="d-flex justify-content-between align-items-center mt-4">
                    <a href="{{ route('generation') }}" class="btn btn-outline-primary"><i
